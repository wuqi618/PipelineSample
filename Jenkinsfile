node {
    ...
    stage('deploy') {
        dir('CloudFormation/') {
            ...
            sh "aws cloudformation create-stack --stack-name webapisample --template-body file://${dir}/ecs.yml --parameters ParameterKey=Image,ParameterValue=${image} ParameterKey=ECSCluster,ParameterValue=${cluster} --region ap-southeast-2"
        }
    }
